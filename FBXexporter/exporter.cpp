...
int main()
{
	...
	FbxScene* scene = LoadFbxSceneFromFile(manager, "/fbx/entrance.fbx");
	printf("FBX 파일 로딩 완료\n");
	...
	FILE* idxOut = fopen("result/idx_계단즈1.txt", "w");
	FILE* frameOut = fopen("result/vtx_걔단즈1.txt", "w");
	...
	*/
	...
	float xscale = 100.0f/ (xmax - xmin);
	float yscale = 10.0f/(max - min);
	float zscale = 10.0f/(zmax - zmin);
