...
int main()
{
	...
	FbxScene* scene = LoadFbxSceneFromFile(manager, "/fbx/PalletTruck.fbx");
	printf("FBX 파일 로딩 완료\n");
	...
	FILE* idxOut = fopen("result/idx_잔해트럭.txt", "w");
	FILE* frameOut = fopen("result/vtx_잔해트럭.txt", "w");
	...
	*/
	...
	float xscale = 8.0f/ (xmax - xmin);
	float yscale = 8.0f/(max - min);
	float zscale = 3.0f/(zmax - zmin);
