...
int main()
{
	...
	FbxScene* scene = LoadFbxSceneFromFile(manager, "/fbx/Large Tank.fbx");
	printf("FBX 파일 로딩 완료\n");
	...
	FILE* idxOut = fopen("result/idx_둥글뱅이탱크.txt", "w");
	FILE* frameOut = fopen("result/vtx_둥글뱅이탱크.txt", "w");
	...
	*/
	...
	float xscale = 50.0f/ (xmax - xmin);
	float yscale = 15.0f/(max - min);
	float zscale = 50.0f/(zmax - zmin);
