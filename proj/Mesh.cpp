...
,float moveX, float moveY, float moveZ) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 36;
	...
	float mx = moveX, my = moveY, mz = moveZ;

	CTexturedVertex pVertices[36];
	int i = 0;

	pVertices[i++] = CTexturedVertex(XMFLOAT3(-fx+mx, +fy+my, -fz+mz), XMFLOAT2(0.0f, 0.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(+fx+mx, +fy + my, -fz + mz), XMFLOAT2(1.0f, 0.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(+fx + mx, -fy + my, -fz + mz), XMFLOAT2(1.0f, 1.0f));

	pVertices[i++] = CTexturedVertex(XMFLOAT3(-fx + mx, +fy + my, -fz + mz), XMFLOAT2(0.0f, 0.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(+fx + mx, -fy + my, -fz + mz), XMFLOAT2(1.0f, 1.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(-fx + mx, -fy + my, -fz + mz), XMFLOAT2(0.0f, 1.0f));
